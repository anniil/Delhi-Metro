...
void Create_Metro_map() {
    keys.push_back("Noida Sector 62~B");
    keys.push_back("Botanical Garden~B");
    keys.push_back("Yamuna Bank~B");
    keys.push_back("Rajiv Chowk~BY");
    keys.push_back("Vaishali~B");
    keys.push_back("Moti Nagar~B");
    keys.push_back("Janak Puri West~BO");
    keys.push_back("Dwarka Sector 21~B");
    keys.push_back("Huda City Center~Y");
    keys.push_back("Saket~Y");
    keys.push_back("Vishwavidyalaya~Y");
    keys.push_back("Chandni Chowk~Y");
    keys.push_back("New Delhi~YO");
    keys.push_back("AIIMS~Y");
    keys.push_back("Shivaji Stadium~O");
    keys.push_back("DDS Campus~O");
    keys.push_back("IGI Airport~O");
    keys.push_back("Rajouri Garden~BP");
    keys.push_back("Netaji Subhash Place~PR");
    keys.push_back("Punjabi Bagh West~P");
...
int main() {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            y--;
            int p = shortestdistance(x, y);
            cout << "Shortest time to reach from source to destination is " << p << " Minutes\n";
        } else if (choice == 5) {
            cout << "Enter the serial no. of Source and Destination" << endl;
            displaystations();
            int x, y;
